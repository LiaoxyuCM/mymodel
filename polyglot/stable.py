        ...
        with open(args.PYfile, encoding=args.Encoding) as pf:
            ...
            if jap:
                if ("'''" in cojfcontent or '"""' in cojfcontent):
                    raise Warning("Cannot contain three double quotes or three single quotes in this JavaScript file. It is recommended to use string concatenation.")
                if "*/" in pfcontent:
                    raise Warning("Cannot contain '*/' in this Python file, it is recommended to use string concatenation.")
    ...
    if args.CopyResult:
        ...
        try:
            ...
            if input("Module 'pyperclip' not found. To copy the result, this module is required. Install it? [Y/n]> ").lower() == "y":
                __import__("os").system("pip install pyperclip")
            else:
...
def compile(polyglotwith: PolyglotWith, JSorCPPcode: str, PYcode: str, CopyResult: bool = False, ReturnFormat: Format = Format.plain):
    ...
    if CopyResult:
        ...
        try:
            ...
            # Fix ModuleNotFoundError
            if input("Module 'pyperclip' not found. To copy the result, this module is required. Would you like to run \"pip install pyperclip\"? [Y/n]> ").lower() == "y":
                print("Failed to install pyperclip, please check it" if __import__("os").system("pip install pyperclip") else "Successfully installed pyperclip. Please re-enter this program.")
            else:
                raise ModuleNotFoundError("Module \"pyperclip\" not found")
