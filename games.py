...
class BeadGameConsole:
    def __init__(self, pattern: list[int] = [0, 0, 2]) -> None:
        self.pattern = pattern

class Player:
    def __init__(self, initCoinsCount: int = 30, defaultGameConsole: BeadGameConsole = BeadGameConsole()) -> None:
        self.coins = initCoinsCount
        self.gc = defaultGameConsole # GameConsole
        self.beads = 0
        self.__cpn = 0
    ...
    def play(self, GameConsole: BeadGameConsole | None = None):
        if self.__cpn:
            self.__cpn -= 1
            gc = GameConsole if GameConsole else self.gc
            c = choice(gc.pattern)
            self.beads += c

def main() -> None:
    bc = BeadGameConsole()
    xiaoming = Player(30, bc)
    xiaoming.DropACoin()
    print(xiaoming.beads)
    xiaoming.DropABead()
    xiaoming.play()
    print(xiaoming.beads)
